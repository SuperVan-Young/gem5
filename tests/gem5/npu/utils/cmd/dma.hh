...
static inline uint32_t
dma_transpose_mode_cfg(uintptr_t src_base, uintptr_t dst_base,
                       uint32_t transpose_dim_a, uint32_t transpose_dim_b)
{
    return (dma_mem_space_for_base(src_base) & 0x1U) |
           ((dma_mem_space_for_base(dst_base) & 0x1U) << 1) |
           ((transpose_dim_a & 0x3U) << 2) |
           ((transpose_dim_b & 0x3U) << 4);
}

static inline uint32_t
dma_transpose_bank_cfg(uint32_t src_bank_id, uint32_t dst_bank_id)
{
    return (src_bank_id & 0xfU) | ((dst_bank_id & 0xfU) << 4);
}

static inline void
dma_cmd_init_transpose(NpuCmd *cmd, uint32_t device_id,
                       uintptr_t src_base, uintptr_t dst_base,
                       const DmaLayout *src_layout,
                       const DmaLayout *dst_layout,
                       uint32_t transpose_dim_a, uint32_t transpose_dim_b,
                       ...
                       uint32_t set_completion_sync)
{
    ...
    dma_cmd.header = npuBuildHeaderWord(
        NPU_DEVICE_TYPE_DMA, device_id,
        (0U << 5) | (DMA_MODE_TRANSPOSE << 2), sync_indicator,
        set_completion_sync ? 1U : 0U, 0U);
    dma_cmd.src_base_addr = (uint32_t)src_base;
    dma_cmd.dst_base_addr = (uint32_t)dst_base;
    dma_cmd.shape_h = src_layout->h;
    ...
    dma_cmd.block_cfg =
        ((uint32_t)dst_layout->k << 16) | src_layout->k;
    dma_cmd.mode_cfg = dma_transpose_mode_cfg(
        src_base, dst_base, transpose_dim_a, transpose_dim_b);
    dma_cmd.bank_cfg = dma_transpose_bank_cfg(src_bank_id, dst_bank_id);
    dma_cmd.word15 = 0U;
    npuBinaryDataFromObject(&binary, dma_cmd);
    cmd->loadBinary(binary);
}
...
static inline void
dma_cmd_init_transpose_stage(NpuCmd *cmd, uint32_t device_id,
                             uintptr_t src_base, uintptr_t dst_base,
                             const DmaLayout *src_layout,
                             const DmaLayout *dst_layout,
                             uint32_t transpose_dim_a,
                             uint32_t transpose_dim_b,
                             uint32_t src_bank_id, uint32_t dst_bank_id,
                             uint32_t command_stage,
                             uint32_t sync_indicator,
                             uint32_t set_completion_sync)
{
    ...
    dma_cmd.header = npuBuildHeaderWord(
        NPU_DEVICE_TYPE_DMA, device_id,
        (0U << 5) | (DMA_MODE_TRANSPOSE << 2) | (command_stage & 0x3U),
        sync_indicator, set_completion_sync ? 1U : 0U, 0U);
    dma_cmd.src_base_addr = (uint32_t)src_base;
    dma_cmd.dst_base_addr = (uint32_t)dst_base;
    dma_cmd.shape_h = src_layout->h;
...
static inline void
dma_cmd_launch_transpose(uint32_t device_id, uintptr_t src_base,
                         uintptr_t dst_base, const DmaLayout *src_layout,
                         const DmaLayout *dst_layout,
                         uint32_t transpose_dim_a, uint32_t transpose_dim_b,
                         uint32_t src_bank_id, uint32_t dst_bank_id,
...
{
    NpuCmd cmd;
    dma_cmd_init_transpose(&cmd, device_id, src_base, dst_base,
                           src_layout, dst_layout, transpose_dim_a,
                           transpose_dim_b, src_bank_id, dst_bank_id,
                           sync_indicator, set_completion_sync);
    cmd.launchCmd();
}

static inline void
dma_cmd_launch_transpose_stage(uint32_t device_id, uintptr_t src_base,
                               uintptr_t dst_base,
                               const DmaLayout *src_layout,
                               const DmaLayout *dst_layout,
                               uint32_t transpose_dim_a,
...
{
    NpuCmd cmd;
    dma_cmd_init_transpose_stage(
        &cmd, device_id, src_base, dst_base, src_layout, dst_layout,
        transpose_dim_a, transpose_dim_b, src_bank_id, dst_bank_id,
        command_stage, sync_indicator, set_completion_sync);
    cmd.launchCmd();
}
