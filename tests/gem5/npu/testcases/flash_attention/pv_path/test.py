...
binary = resolve_binary_path(__file__, "flash_attention_pv_path_riscv")

register_npu_test(
    NpuRunnerSpec(
        name="flash_attention_pv_path",
        config=resolve_config_path(__file__),
        config_args=tuple(
            make_binary_config_args(
                binary, "--scenario", "flash_attention_pv_path"
            )
        ),
        gem5_args=(
        ...
        verifier_specs=(
            r"MPU_SUMMARY scenario=flash_attention_pv_path cmds=28 "
            r"reads=26 writes=6 .* mvin=8 load=8 compute=4 drain=4 mvout=4 .*",
            r"FLASH_ATTENTION_PV_PATH_PASS",
            r"MPU_EXIT_CODE=0",
        ),
