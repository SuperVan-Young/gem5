...
binary = resolve_binary_path(
    __file__, "flash_attention_dequantize_i32_to_f32_riscv"
)

register_npu_test(
    NpuRunnerSpec(
        name="flash_attention_dequantize_i32_to_f32",
        config=resolve_config_path(__file__),
        config_args=tuple(make_binary_config_args(binary)),
        verifier_specs=(
            r"FLASH_ATTENTION_DEQUANTIZE_I32_TO_F32_PASS",
            r"FLASH_ATTENTION_DEQUANTIZE_I32_TO_F32_CONFIG_PASS",
        ),
