...
binary = resolve_binary_path(__file__, "flash_attention_layout_planner_riscv")

register_npu_test(
    NpuRunnerSpec(
        name="flash_attention_layout_planner",
        config=resolve_config_path(__file__),
        config_args=tuple(make_binary_config_args(binary)),
        verifier_specs=(
            r"FLASH_ATTENTION_LAYOUT_PLANNER_PASS",
            r"FLASH_ATTENTION_LAYOUT_PLANNER_CONFIG_PASS",
        ),
