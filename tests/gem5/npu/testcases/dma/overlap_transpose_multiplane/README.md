...
## 仿真程序
程序构造一个 `8x8x8` 的源 tensor，按 `W <-> C` transpose 发射 staged `Load`、`Compute`、`Store` 三条命令。该几何会产生多个 transpose iteration，且 overlap-positive 路径选择了 destination-line 不冲突的批次组合，因此既能观察 overlap，也不会把 destination-line hazard 混进这个验证里。case 先检查最终目标 tensor，再检查 DMA-local overlap 计数器。
