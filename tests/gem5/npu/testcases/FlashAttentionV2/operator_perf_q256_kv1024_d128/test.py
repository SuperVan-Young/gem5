...
class GeneratePerformanceSummary(verifier.Verifier):
    ...
    def test(self, params):
        ...
        if summary["tiling"]["attention_tiles"] != 16:
            test_util.fail("Expected 16 BR/BC attention tiles")
        if summary["profile"]["qk_fast_matmul"]["macro_count"] != 16:
            test_util.fail("QK fastMatmul should have 16 macros")
        if summary["profile"]["pv_fast_matmul"]["macro_count"] != 16:
            test_util.fail("PV fastMatmul should have 16 macros")
        if summary["profile"]["fast_online_softmax"]["macro_count"] != 224:
            test_util.fail("fastOnlineSoftmax should have 224 macros")
        if summary["profile"]["operator_total"]["macro_count"] != 256:
            test_util.fail("FlashAttentionV2 operator should have 256 macros")
        if summary["mpu_summary"]["cmds"] != 32:
            test_util.fail("Expected 32 MPU fastMatmul commands")
        if summary["vpu_summary"]["cmds"] != 224:
            test_util.fail("Expected 224 VPU softmax commands")
...
register_npu_test(
    NpuRunnerSpec(
        ...
        verifier_specs=(
            ...
            r"FLASH_ATTENTION_V2_QK_FAST_MATMUL cmds=16 template_builds=16 "
            r"sync_indicator=129 status=PASS",
            r"FLASH_ATTENTION_V2_FAST_ONLINE_SOFTMAX cmds=224 "
            r"sync_indicator=145 status=PASS",
            r"FLASH_ATTENTION_V2_PV_FAST_MATMUL cmds=16 template_builds=16 "
            r"sync_indicator=130 status=PASS",
            r"FLASH_ATTENTION_V2_PROFILE_QK total_span_cycles=[0-9]+ "
            r"busy_cycles=[0-9]+ macro_count=16 fused_matmul=16 status=PASS",
            r"FLASH_ATTENTION_V2_PROFILE_SOFTMAX total_span_cycles=[0-9]+ "
            r"busy_cycles=[0-9]+ macro_count=224 reduce_max=16 add=48 sub=48 "
            r"mul=16 div=16 scale=16 abs=16 reduce_sum=16 exp=32 status=PASS",
            r"FLASH_ATTENTION_V2_PROFILE_PV total_span_cycles=[0-9]+ "
            r"busy_cycles=[0-9]+ macro_count=16 fused_matmul=16 status=PASS",
            r"FLASH_ATTENTION_V2_PROFILE_TOTAL total_span_cycles=[0-9]+ "
            r"busy_cycles=[0-9]+ macro_count=256 status=PASS",
            r"FLASH_ATTENTION_V2_PASS",
            r"FLASH_ATTENTION_V2_MPU_SUMMARY cmds=32 "
            r"compute_cycles=[0-9]+ drain_cycles=[0-9]+ "
            r"output_ready_cycles=[0-9]+ cmd_cycles=[0-9]+ "
            r"spm_wait=[0-9]+ macs=[0-9]+ busy=[0-9]+ idle=[0-9]+",
            r"FLASH_ATTENTION_V2_VPU_SUMMARY cmds=224 queue_occupancy=0 "
